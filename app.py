...
# ─── Path Config (robust untuk lokal & Streamlit Cloud) ─────
# __file__ bisa berada di root repo ATAU di subfolder app/
_THIS_FILE = os.path.abspath(__file__)
_THIS_DIR  = os.path.dirname(_THIS_FILE)

# Cari folder "model" mulai dari lokasi file ini, naik max 2 level
def _find_model_dir():
    for base in [_THIS_DIR,
                 os.path.dirname(_THIS_DIR),
                 os.path.dirname(os.path.dirname(_THIS_DIR))]:
        candidate = os.path.join(base, "model")
        if os.path.isdir(candidate) and os.path.exists(os.path.join(candidate, "rf_model.pkl")):
            return candidate
    # Fallback: relatif terhadap current working directory
    cwd_candidate = os.path.join(os.getcwd(), "model")
